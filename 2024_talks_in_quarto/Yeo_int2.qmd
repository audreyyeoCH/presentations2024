...
footer: "Audrey Yeo's DSS Interview"
---
...
- Trial Simulation / Operating Characteristics
...
<!-- -   graduated MSc Biostatistics end 2000, Uni of Zürich -->
<!-- -   Registered Nurse, `Royal Children's Hospital`, COVID ICU, early 2021 -->

\n
\n
-   joined GPS at Roche mid 2021

        - RWD Enabling platform (Statistical Computing Environment Apollo, Flatiron and other RWD assets)
-   joined DSS at Roche in mid 2022

        - Project Lead Statistician in Early Oncology Trials
        - Study Statistician for phase I-II, phase III trials
        - First Roche/Genentech Dose Escalation on new SCE (OCEAN)
        - Lead the development of R package "phase1b"
        - Julia Course Basel Instructor (Data Science, Quarto, ML modules)
        - Co-organiser Basel Biostats Forum/ Knowledge Sharing
        - Many presentations (Tech Talks / BBF / Knowledge Sharing)
        
<!-- -   first internal statistics presentation on Decision gating in  -->
<!--     `Basel Knowledge Sharing` November 2022 -->
<!-- -   started `phase1b` in July 2023 -->
-   `phase1b`'s world debut at PSI 2024 followed by:

        - useR!2024 
        - R/Pharma 2024
        - Effective Statistician conference and more!
...
# Statisticians are Strategic partners in drug development :{.smaller}

...we understand the combined Scientific and Business development of a therapy such as :

  - Roadblocks to end points e.g. recruitment rates and failures,
business assumptions and strategies
  - Drug pharmacokinetics and pharmacodynamics profiles e.g. latent drug effect
  - Patient population e.g. rare diseases
  - Expectations from health authorities (regulators)
  
Developing `phase1b` has **strengthened** my Strategic role as a Statistician
...
set.seed(2025)
res <- ocPostprob(
  nnE = c(23, 40), truep = 0.60, p0 = 0.60, p1 = 0.60, tL = 0.70, tU = 0.90, parE = c(0.6, 0.4),
...
# Thanks{background-iframe="rubik/dist/index.html"} 
...
# Project related milestones{.smaller}

## Oncology Forum/ ESPC
- PLS responsible for Decision making framework for TPP/MVP (2022)
Futility analysis (2023)
- code analysis for Neuroscience Rare Disease
Dose Escalation (2022-2024)
- early Oncology Molecule (FAB4-1BBL, NXT007) 

## Other milestones 
- Advancing practice of Bayesian decision making applications in Oncology, Immunology and Cardiovascular
- Spearheaded doseslidR solution for Roche's first Dose Escalation Study in OCEAN platform
- Lead first Dose Escalation in OCEAN platform in Roche/Genentech
- Knowledge sharing and leading Stats engagement in OCEAN activities

## General community activites : a focus on Methodologies
- Basel Biostatistics Forum co-organiser since 2021
- Basel Knowledge Sharing co-organiser since 2021

## Talks 2024 {.smaller}

```{r}
# | fig-alt: "This table has on the first column other phase1b call functions and on the next columns, have check boxes that show Features like SOC uncertainty, single-arm, two-arm ,simulation , plotting and boundaries" 
talks2024 <- read.csv(file = "Talks2024_2025.csv", header = TRUE, sep  = ",")
talks2024 <- talks2024 %>% slice(-20) 
# names(Features) <- c(" ", "SOC uncertainty", "single-arm", "two-arm", "simulation", "plotting", "boundaries")
# Features[is.na(Features)] <- ""
# Features[Features == 1] <- "✔️"
# what are these features, diff between two arm and randomisation say what kind of trials this can support
talks2024 %>% kbl(align = "c") %>% kable_styling(position = "center", font_size = 15) %>% kable_classic(lightable_options = "hover", html_font = "\"Source Sans Pro\", helvetica, sans-serif") %>% kable_minimal() 
```
...
```{r}
# | fig-alt: "This table has on the first column other phase1b call functions and on the next columns, have check boxes that show Features like SOC uncertainty, single-arm, two-arm ,simulation , plotting and boundaries" 
talks2025 <- read.csv(file = "Talks2024_2025.csv", header = TRUE, sep  = ",")
