...
PDD Tech Talks, Roche EU WLN SSF SH
...
#| fig-subcap:
#|     - $P (\pi > 0.6 | \ interim data )$ > 60% #"Efficacious if Pred. Posterior Prob > 90 %" 
#|     - $P (\pi > 0.6 |  \ interim data )$ > 90% #"Efficacious if Pred. Posterior Prob > 90 %" 
#| column: page-right
...
ggplot(df_thetaTlow) +
  ...
  ylab("Probability\n") +
  ggtitle("With lower threshold : \n10 of remaining patients needed as responders to achieve a Go decision")  +
...
ggplot(df_thetaThigh) +
  geom_point(aes(x = cumul_counts, y = posterior, colour = success)) +
   scale_x_discrete(limits = c(seq(23, 40, by = 1))) +
    xlab("\nFuture successful reponders") +
  ylab("Probability\n") +
  ggtitle("With higher threshold : \n13 of of remaining patients needed to achieve a Go decision") +
  theme(text = element_text(size = 20)) +
  scale_color_manual(values = c("#D55E00", "#009E73")) 
```
...
-   Rationale : Making `phase1b` a State of Art Software[^1] - reproducible, robust, testable, intuitive and open to collaboration
...
*To find a needle in the haystack but systematically*
...
2.  One issue is the smallest task from (3)
...
-   Taking Systematic steps : no matter how slow
    -   e.g. `debug()`, `undebug()`, `options(recover = error)`, `options(recover = NULL)`, `browser()`
...
## Thank you {background-iframe="rubik/index.html"}
