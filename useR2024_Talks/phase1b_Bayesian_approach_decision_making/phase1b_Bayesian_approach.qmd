...
# #| fig-width: 0.5
# #| fig-height: 0.5
# knitr::include_graphics("images/hex6.png", error = FALSE)
# knitr::include_graphics("images/hex3.png", error = FALSE)
# knitr::include_graphics("images/hex9.png", error = FALSE)
# knitr::include_graphics("images/hex1.png", error = FALSE)
# knitr::include_graphics("images/hex5.png", error = FALSE)
```
::: {#layout-nrow=6}
![](images/hex3.png){width=20%} ![](images/hex5.png){width=20%} ![](images/hex6.png){width=20%}
:::
...
```{r}
#| fig-subcap:
#| - 1. Go if $P (\pi > 0.6 | \ interim \ data )$ > 60%  
#"Efficacious if Pred. Posterior Prob > 90 %" 
ggplot(df_thetaTlow) +
  geom_point(aes(x = cumul_counts, y = posterior, colour = success)) +
  scale_x_discrete(limits = c(seq(23, 40, by = 1))) +
  xlab("\nFuture successful reponders") +
  ylab("Probability\n") +
  ggtitle("32 of 40 responders needed to achieve a Go decision")  +
  theme(text = element_text(size = 20)) +
...
control = 0.6
result <- predprob(
  x = 16, n = 23, Nmax = 40, p = control, thetaT = 0.6,
  parE = c(0.6, 0.4)
)
result$result
```
...
Features <- read.csv(file = "../../PSI_2024/phase1b_FeatureTable - Sheet1.csv", header = TRUE, sep  = ",")
...
-   Extension to other disease areas that use response rate as
...
<!-- # Quarto Help -->
<!-- include_graphics("image_path", error = FALSE), setting `error = FALSE` -->
<!-- - https://github.com/yihui/knitr/issues/2171 -->
<!-- - https://github.com/quarto-dev/quarto-cli/discussions/8466 -->
