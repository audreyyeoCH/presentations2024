...
# author: "Audrey Yeo"
# institute: "Basel Biostats Forum, 2024"
format: 
 revealjs:
    code-fold: false
    width: 1280
...
Basel Biostatistics Forum 2024, Basel, CH
\
...
- Using the formula for the mean, the new mean is :
...
data = data.frame(rate = rep(xx, 3),
                  ...
                  Category = c(rep("Uniform Prior", length(xx)), rep("Prior", length(xx)), rep("Updated Posterior", length(xx))))
...
0.5*postprob(x = 16, n = 23, p = 0.60, parE = c(0.6, 0.4)) +
  0.5*postprob(x = 16, n = 23, p = 0.60, parE = c(2, 4), weights = 0.5)

# postprob(x = 16, n = 23, p = 0.60, par = rbind(c(0.6, 0.4), c(2, 4)), weights = c(0.5, 0.5))
```


$$  f(\pi | x_{i})  \propto \  \pi^{x_{i}} (1-\pi)^{n-x_{i}}\sum_{j = 1}^{k} \ w_j \frac {1}{B(\alpha_j, \beta_j)} \pi^{\alpha_j-1}(1-\pi)^{\beta_j-1} 
$$
...
## Posterior probability, and threshold for Success (or failure):
...
- Interim STOP : `P (failure at final ) > phiFu`
 ...
 ## When the standard of care "distribution" is not known. Introducing `postprobDist()`, variations also exist e.g. `predprobDist`, `ocPostprobDist()` and of course `ocPredprobDist()`.
